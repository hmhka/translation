...
IF pprocess-type = 1 THEN DO:  
    IF SEARCH(pspeech-ori-eng-file) = ? THEN DO:
    ...
    INPUT FROM VALUE(pspeech-ori-eng-file).
    REPEAT:
...
IF pprocess-type = 2 THEN DO:  
    IF SEARCH(pspeech-ori-hin-file) = ? THEN DO:
    ...
    INPUT FROM VALUE(pspeech-ori-hin-file).
    REPEAT:
        ASSIGN pcons = pcons + 1.
        ...
        IF available speech_text THEN do:
           IMPORT UNFORMATTED speech_text.speech-corr-hin.
